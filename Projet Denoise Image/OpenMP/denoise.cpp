...
void jacobiDenoise(const Image &input, Image &output, int iterations) {
    ...
    for (int iter = 0; iter < iterations; iter++) { // Pour chaque itération
        #pragma omp parallel // Commence une région parallèle
        {
            #pragma omp single // Assure qu'un seul thread crée les tâches
            {
                for (int i = 0; i < height; i++) {
                    #pragma omp task firstprivate(i) // Crée une tâche pour chaque ligne
                    {
