...
void jacobiDenoise(const Image &input, Image &output, int iterations) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        #pragma omp target teams distribute parallel for collapse(2) map(to: temp.pixels[0:width * height]) map(from: output.pixels[0:width * height]) firstprivate(width, height)
...
void gaussSeidelDenoiseWavefront(const Image &input, Image &output, int iterations) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        #pragma omp target teams distribute parallel for collapse(2) map(to: temp.pixels[0:width * height]) map(tofrom: output.pixels[0:width * height]) firstprivate(width, height)
...
int main(int argc, char **argv) {
    ...
    } // GaussSeideloutput est libéré ici
